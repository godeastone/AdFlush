...
def pipeline(idx, start_row, end_row):
    save_directory = '/data2/shine/AdFlush/source/MY_jsfile/'
    csv_file_path = "/home/shine/PoisoningAttack/dataset/from_adflush/features_raw.csv"
    output_csv_file = "/data2/shine/AdFlush/source/MY_jsfile/a.csv".format(idx)

    tot_num = end_row - start_row
    ...
    with open(csv_file_path, 'r') as file, open(output_csv_file, 'w', newline='') as csv_output:
        ...
        for index, row in enumerate(reader):
            ...
            if start_row <= index <= end_row:
                ...
                if cps == "script":
                    try:
                        ast_depth, ast_breadth, avg_ident, avg_charperline, brackettodot, num_requests_sent, num_set_storage, num_get_storage, num_get_cookie, ngram= extract_JS_Features_shine(file_name=js_name, _isHTML=False)
...
for i in range(int(num_total / per_core + 1)):
    name = "Process" + str(i)
    p = Process(target=pipeline, args=(i, i * per_core, (i + 1) * per_core, ))
    p.start()
    processes.append(p)

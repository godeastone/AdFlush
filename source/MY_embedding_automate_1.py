from adflush_encodings_1 import *
import csv
import urllib.parse
import os
import requests
...
def copy_js_file(url, data_dict, data_dict2):
    try:
        ...
        copyfile("/yopo-artifact/data/rendering_stream/saved_js_adflush_{}/{}".format(shell,js_fname), "/yopo-artifact/AdFlush/source/MY_jsfile_{}/{}".format(shell, js_fname))
...
def pipeline(mapping_list):
    save_directory = '/yopo-artifact/AdFlush/source/MY_jsfile_{}/'.format(shell)
    csv_file_path = "/yopo-artifact/data//dataset_adflush/modified_features_target_all_adflush_{}.csv".format(shell)
    output_csv_file = "/yopo-artifact/data/dataset_adflush/modified_features_target_all_adflush_{}_final.csv".format(shell)
            ...
            if True:
                ...
                if cps == "script":
                    ...
                    try:
                        ...
                        pass
                    try:
                        ast_depth, ast_breadth, avg_ident, avg_charperline, brackettodot, num_requests_sent, num_set_storage, num_get_storage, num_get_cookie, ngram= extract_JS_Features_shine(file_name=js_name, _isHTML=False, timeout=180)
                    except Exception as err:
                        # print(err)
                        # print("err5")
                        ast_depth = 0
...
final_url_to_html_mapping = "/yopo-artifact/scripts/dataset/from_adflush/target_features_rf_full.csv"
mapping_list = create_map_list(final_url_to_html_mapping)

csv_name = "/yopo-artifact/data/dataset_adflush/modified_features_target_all_adflush_{}.csv".format(shell)
processes = []
...
csv_file_filtered = "/yopo-artifact/data//dataset_adflush/modified_features_target_all_adflush_{}_final.csv".format(shell)
output_file_filtered = "/yopo-artifact/data//dataset_adflush/modified_features_target_all_adflush_{}_final_filtered.csv".format(shell)
